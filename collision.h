...
class AABB 
{
    ...
    void draw_AABB(Shader& shader)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, AABBvertices.size() * sizeof(glm::vec3), &AABBvertices[0], GL_STATIC_DRAW);
        
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, AABBindices.size() * sizeof(unsigned int), &AABBindices[0], GL_STATIC_DRAW);
        ...
        // 绘制碰撞箱
        glBindVertexArray(VAO);
        glDrawElements(GL_LINES, AABBindices.size(), GL_UNSIGNED_INT, 0);
        glBindVertexArray(0);
